...
<script>
  ...
  const tableData = {
    1: { text: "🟢 低地震風險區", color: "#28a745", tw: ["無要求", "不開裂", "ϕ=0.75"], us: ["SDC A", "不開裂", "ϕ=0.75"], eu: ["Static", "不開裂", "γ=1.5"] },
    2: { text: "🟡 中地震風險區", color: "#ffc107", tw: ["建議C1", "0.3mm", "ϕ=0.75"], us: ["SDC C", "強制開裂", "ϕ=0.75"], eu: ["C1等級", "強制開裂", "γ=1.5"] },
    3: { text: "🔴 高地震風險區", color: "#D21F3C", tw: ["強制C2", "0.5mm", "0.75²"], us: ["SDC D/F", "嚴格開裂", "0.75²"], eu: ["C2等級", "循環開裂", "γ=折減"] }
  };
  const rows = ["抗震等級", "開裂設定", "強度折減"];
  function updateTable(val) {
    riskText.innerHTML = tableData[val].text;
    riskText.style.color = tableData[val].color;
    let html = "";
    for(let i=0; i<rows.length; i++){
      html += `<tr><td style="font-weight:bold;">${rows[i]}</td><td>${tableData[val].tw[i]}</td><td>${tableData[val].us[i]}</td><td>${tableData[val].eu[i]}</td></tr>`;
    }
